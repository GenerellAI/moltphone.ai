/**
 * Integration tests for the dial protocol — POST /dial/:phoneNumber/tasks/send
 *
...
import { POST as tasksSend } from '../../app/dial/[phoneNumber]/tasks/send/route';
...
describe('POST /dial/:phoneNumber/tasks/send — basic', () => {
  it('queues task for offline public agent', async () => {
    const agent = buildMockAgent();
    mockPrisma.agent.findUnique.mockResolvedValue(agent);

    const req = buildRequest('POST', `/dial/${agent.phoneNumber}/tasks/send`, {
      body: validTaskBody(),
    });
    const res = await tasksSend(req, { params: Promise.resolve({ phoneNumber: agent.phoneNumber }) });
  ...
  it('returns 404 for non-existent agent', async () => {
    ...
    const req = buildRequest('POST', '/dial/MOLT-XXXX-YYYY-ZZZZ-AAAA/tasks/send', {
      body: validTaskBody(),
    });
    const res = await tasksSend(req, { params: Promise.resolve({ phoneNumber: 'MOLT-XXXX-YYYY-ZZZZ-AAAA' }) });

    expect(res.status).toBe(404);
  });

  it('returns 403 when dialling is disabled', async () => {
    const agent = buildMockAgent({ dialEnabled: false });
    mockPrisma.agent.findUnique.mockResolvedValue(agent);
...
describe('POST /dial/:phoneNumber/tasks/send — DND', () => {
  ...
  it('sends push notification on DND if push endpoint set', async () => {
    ...
    await tasksSend(req, { params: Promise.resolve({ phoneNumber: agent.phoneNumber }) });
...
describe('POST /dial/:phoneNumber/tasks/send — busy', () => {
...
describe('POST /dial/:phoneNumber/tasks/send — rate limit', () => {
...
describe('POST /dial/:phoneNumber/tasks/send — policy', () => {
...
describe('POST /dial/:phoneNumber/tasks/send — carrier blocks', () => {
  it('rejects blocked callers', async () => {
    const agent = buildMockAgent();
    const callerKp = generateKeyPair();
    const callerPhone = generatePhoneNumber('MOLT', callerKp.publicKey);
    ...
    const req = buildRequest('POST', `/dial/${agent.phoneNumber}/tasks/send`, {
      body: validTaskBody(),
      headers: { 'x-molt-caller': callerPhone },
    });
    const res = await tasksSend(req, { params: Promise.resolve({ phoneNumber: agent.phoneNumber }) });
...
describe('POST /dial/:phoneNumber/tasks/send — per-agent blocks', () => {
  it('rejects blocked callers', async () => {
    const agent = buildMockAgent();
    const callerKp = generateKeyPair();
    const callerPhone = generatePhoneNumber('MOLT', callerKp.publicKey);
...
describe('POST /dial/:phoneNumber/tasks/send — webhook', () => {
  beforeEach(() => {
...
describe('POST /dial/:phoneNumber/tasks/send — carrier_only relay', () => {
...
describe('POST /dial/:phoneNumber/tasks/send — push notifications', () => {
...
describe('POST /dial/:phoneNumber/tasks/send — error codes', () => {
