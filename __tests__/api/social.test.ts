/**
 * Integration tests for /api/favorites and /api/blocks routes.
 */
...
import { GET as listFavorites, POST as addFavorite } from '../../app/api/favorites/route';
...
describe('GET /api/favorites', () => {
  it('returns user favorites', async () => {
    ...
    mockPrisma.favorite.findMany.mockResolvedValue([
      { id: 'fav-1', userId: TEST_USER.id, agentId: agent.id, agent, createdAt: new Date() },
    ]);

    const req = buildRequest('GET', '/api/favorites');
...
describe('POST /api/favorites', () => {
  it('adds agent to favorites', async () => {
    const agent = buildMockAgent();
    ...
    const req = buildRequest('POST', '/api/favorites', {
