...
const MOLTNUMBER_RE = /^MOLT-[0-9A-HJ-NP-Z]{4}-[0-9A-HJ-NP-Z]{4}-[0-9A-HJ-NP-Z]{4}-[0-9A-HJ-NP-Z]{4}$/;
...
async function callbackTest(callerNumber, dialBase) {
  if (!callerNumber || callerNumber === 'anonymous') return { ok: false, reason: 'No caller number' };
  const base = dialBase || moltSim.carrier_dial_base;
  if (!base) return { ok: false, reason: 'No dial base URL in MoltSIM' };
  try {
    const baseUrl = new URL(base);
...
const defaultCarrierDomain = (() => {
  ...
  if (typeof moltSim.carrier_dial_base === 'string' && moltSim.carrier_dial_base) return new URL(moltSim.carrier_dial_base).hostname;
