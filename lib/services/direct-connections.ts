 ...
 *    Allocation. The carrier relays ALL traffic. The agent's call URL is
...
interface ProposeError {
  ok: false;
  reason: string;
  code: 'policy_denied' | 'already_exists' | 'no_endpoint' | 'self_connection' | 'not_found';
}
...
export async function listDirectConnections(
  ...
  return prisma.directConnection.findMany({
    ...
    include: {
      proposerAgent: { select: { id: true, displayName: true, moltNumber: true } },
      targetAgent: { select: { id: true, displayName: true, moltNumber: true } },
    },
